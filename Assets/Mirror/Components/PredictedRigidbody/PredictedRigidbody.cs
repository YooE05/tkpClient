...
namespace Mirror
{
    ...
    public class PredictedRigidbody : NetworkBehaviour
    {
        ...
        Transform
            predictedRigidbodyTransform; // predictedRigidbody.transform for performance (Get/SetPositionAndRotation)
        ...
        public float
            motionSmoothingAngularVelocityThreshold =
                5.0f; // Billiards demo: 0.1 is way too small, takes forever for IsMoving()==false
        ...
        [Header("State History")] public int stateHistoryLimit = 32; // 32 x 50 ms = 1.6 seconds is definitely enough
        ...
        [Tooltip(
            "(Optional) performance optimization where received state is compared to the LAST recorded state first, before sampling the whole history.\n\nThis can save significant traversal overhead for idle objects with a tiny chance of missing corrections for objects which revisisted the same position in the recent history twice.")]
        public bool compareLastFirst = true;

        [Header("Reconciliation")]
        [Tooltip(
            "Correction threshold in meters. For example, 0.1 means that if the client is off by more than 10cm, it gets corrected.")]
        public double positionCorrectionThreshold = 0.10;

        double positionCorrectionThresholdSqr; // ² cached in Awake

        [Tooltip(
            "Correction threshold in degrees. For example, 5 means that if the client is off by more than 5 degrees, it gets corrected.")]
        public double rotationCorrectionThreshold = 5;

        [Tooltip(
            "Applying server corrections one frame ahead gives much better results. We don't know why yet, so this is an option for now.")]
        public bool oneFrameAhead = true;

        [Header("Smoothing")]
        [Tooltip(
            "Snap to the server state directly when velocity is < threshold. This is useful to reduce jitter/fighting effects before coming to rest.\nNote this applies position, rotation and velocity(!) so it's still smooth.")]
        public float snapThreshold = 2; // 0.5 has too much fighting-at-rest, 2 seems ideal.

        [Header("Visual Interpolation")]
        [Tooltip(
            "After creating the visual interpolation object, keep showing the original Rigidbody with a ghost (transparent) material for debugging.")]
        public bool showGhost = true;

        [Tooltip(
            "Physics components are moved onto a ghost object beyond this threshold. Main object visually interpolates to it.")]
        public float ghostVelocityThreshold = 0.1f;

        [Tooltip(
            "After creating the visual interpolation object, replace this object's renderer materials with the ghost (ideally transparent) material.")]
        ...
        [Header("Bandwidth")]
        [Tooltip(
            "Reduce sends while velocity==0. Client's objects may slightly move due to gravity/physics, so we still want to send corrections occasionally even if an object is idle on the server the whole time.")]
        ...
        protected virtual void Awake()
        {
            ...
            if (predictedRigidbody == null)
                throw new InvalidOperationException($"Prediction: {name} is missing a Rigidbody component.");
            ...
            motionSmoothingAngularVelocityThresholdSqr =
                motionSmoothingAngularVelocityThreshold * motionSmoothingAngularVelocityThreshold;
        ...
        protected virtual void CopyRenderersAsGhost(GameObject destination, Material material)
        {
            ...
            // if we didn't find a renderer, show a warning
                ...
                Debug.LogWarning(
                    $"PredictedRigidbody: {name} found no renderer to copy onto the visual object. If you are using a custom setup, please overwrite PredictedRigidbody.CreateVisualCopy().");
        }
        ...
        protected virtual void CreateGhosts()
        {
            ...
            PredictedRigidbodyPhysicsGhost physicsGhostRigidbody =
                physicsCopy.AddComponent<PredictedRigidbodyPhysicsGhost>();
            ...
            physicsGhostRigidbody.transform.localScale =
                tf.lossyScale; // world scale! // = initialScale; // don't change scale for now. causes issues with parenting.
        ...
        protected virtual void SmoothFollowPhysicsCopy()
        {
            ...
            */
            ...
            // tf.GetPositionAndRotation(out Vector3 currentPosition,
            //      out Quaternion currentRotation); // faster than tf.position + tf.rotation
            var currentPosition = tf.position;
            var currentRotation = tf.rotation;
            //  predictedRigidbodyTransform.GetPositionAndRotation(out Vector3 physicsPosition,
            //      out Quaternion physicsRotation); // faster than Rigidbody .position and .rotation

            var physicsPosition = predictedRigidbodyTransform.position;
            var physicsRotation = predictedRigidbodyTransform.rotation;
            ...
            {
                tf.SetPositionAndRotation(physicsPosition,
                    physicsRotation); // faster than .position and .rotation manually
                Debug.Log(
                    $"[PredictedRigidbody] Teleported because distance to physics copy = {distance:F2} > threshold {smoothFollowThreshold:F2}");
                return;
            }
            ...
            Vector3 newPosition = MoveTowardsCustom(currentPosition, physicsPosition, delta, sqrDistance, distance,
                positionStep * deltaTime);

            // smoothly interpolate to the target rotation.
            // Quaternion.RotateTowards doesn't seem to work at all, so let's use SLerp.
            // Quaternions always need to be normalized in order to be a valid rotation after operations
            Quaternion newRotation = Quaternion
                .Slerp(currentRotation, physicsRotation, rotationInterpolationSpeed * deltaTime).normalized;
        ...
        void RecordState()
        {
            ...
            // tf.GetPositionAndRotation(out Vector3 currentPosition,
            //    out Quaternion currentRotation); // faster than accessing .position + .rotation manually
        ...
        {
        } // when the Rigidbody moved above threshold and we created a ghost

        protected virtual void OnEndPrediction()
        {
        } // when the Rigidbody came to rest and we destroyed the ghost
        ...
        void OnReceivedState(double timestamp, RigidbodyState state) //, bool sleeping)
        {
            ...
            if (remoteCopy != null)
            {
                ...
                remoteCopyTransform.SetPositionAndRotation(state.position,
                    state.rotation); // faster than .position + .rotation setters
            ...
            // predictedRigidbodyTransform.GetPositionAndRotation(out Vector3 physicsPosition,
            //     out Quaternion physicsRotation);
            ...
            if (state.timestamp < oldest.timestamp)
            {
                ...
                if (stateHistory.Count >= stateHistoryLimit)
                    Debug.LogWarning(
                        $"Hard correcting client object {name} because the client is too far behind the server. History of size={stateHistory.Count} @ t={timestamp:F3} oldest={oldest.timestamp:F3} newest={newest.timestamp:F3}. This would cause the client to be out of sync as long as it's behind.");
            ...
            if (!Prediction.Sample(stateHistory, timestamp, out RigidbodyState before, out RigidbodyState after,
                out int afterIndex, out double t))
            {
                // something went very wrong. sampling should've worked.
                // hard correct to recover the error.
                Debug.LogError(
                    $"Failed to sample history of size={stateHistory.Count} @ t={timestamp:F3} oldest={oldest.timestamp:F3} newest={newest.timestamp:F3}. This should never happen because the timestamp is within history.");
                ApplyState(state.timestamp, state.position, state.rotation, state.velocity, state.angularVelocity);
                return;
            }
            ...
            float positionToInterpolatedDistanceSqr =
                Vector3.SqrMagnitude(state.position - interpolated.position); // fast comparison
                ...
                rotationToInterpolatedDistance >= rotationCorrectionThreshold)
            {
                ...
                RigidbodyState recomputed =
                    Prediction.CorrectHistory(stateHistory, stateHistoryLimit, state, before, after, afterIndex);
        ...
        public override void OnSerialize(NetworkWriter writer, bool initialState)
        {
            ...
            // FAST VERSION: this shows in profiler a lot, so cache EVERYTHING!
            // tf.GetPositionAndRotation(out Vector3 position,
            //     out Quaternion rotation); // faster than tf.position + tf.rotation. server's rigidbody is on the same transform.
        ...
        public override void OnDeserialize(NetworkReader reader, bool initialState)
        {
            ...
            OnReceivedState(timestamp,
                new RigidbodyState(timestamp, Vector3.zero, position, Quaternion.identity, rotation, Vector3.zero,
                    velocity, Vector3.zero, angularVelocity)); //, sleeping);
        }
